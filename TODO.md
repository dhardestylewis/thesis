# Thesis Project TODO

## Priority Tasks

### P1 - Before Spring Draft
- [ ] Full audit of ALL `\cite{}` commands in Background-COMPREHENSIVE.tex against references.bib
- [ ] Identify any remaining undefined citations
- [ ] Organize `references.bib` with commented section headers:
  - Primary Sources - Government Documents (City, State, Federal, Court)
  - Secondary Sources - Academic Research (Books, UT Research, Other)
  - News Sources and Journalistic Accounts
  - Organizational Sources (Advocacy, Think Tanks)
  - Data Sources
  - Legal and Regulatory Sources

### P2 - Spring Thesis Draft
- [ ] Tables 3-7 from `Tables_and_Figures-TO_INTEGRATE.md` — add when drafting thesis
- [ ] Figures 1-5 from `Tables_and_Figures-TO_INTEGRATE.md` — add when drafting thesis
- [ ] Cross-reference figure captions with narrative text
- [ ] Verify all cited references point to accurate, authoritative sources

### P3 - Cleanup
- [ ] Rename `Austin_Housing_Bibliography.md` and decide what to do with it (deprecate or integrate)
- [ ] Consider consolidating duplicate bib entries for same facts

## Completed (Dec 11, 2025)
- [x] Added missing bib entries: hb2127, thirdcourt2025, acuna2023, axios2023parking, shoup2024, kut2023parking, sanjose2022parking
- [x] Fixed single-stair date: July 10 → Sept 1, 2025
- [x] Verified HB 2127 Third Court ruling date: July 18, 2025
- [x] Verified San José parking elimination: Dec 6, 2022
- [x] Verified Austin parking elimination: Nov 2, 2023
