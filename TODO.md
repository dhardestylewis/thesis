...
- [ ] (UNREVIEWED) Organize `references.bib` with commented section headers:
...
- [ ] (UNREVIEWED) Verify all cited references point to accurate, authoritative sources

### P3 - Cleanup
- [ ] (UNREVIEWED) Rename `Austin_Housing_Bibliography.md` and decide what to do with it (deprecate or integrate)
- [ ] (UNREVIEWED) Consider consolidating duplicate bib entries for same facts
- [ ] (UNREVIEWED) Replace `[Author Name]` placeholder in all .tex files with "Daniel Hardesty Lewis"
  - `Background-COMPREHENSIVE.tex` (line 26)
  - `Deprecated_Writings/` files (not critical, but for consistency)
- [ ] (UNREVIEWED) Update `Tables_and_Figures-TO_INTEGRATE.md` "Pending" items when legislation passes:
