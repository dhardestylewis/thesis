...
## 🧹 Draft Hygiene & Duplicate Prevention (CRITICAL)

1.  **Check Before Creating:** Before running ANY draft creation script, scan the current drafts folder. If drafts already exist for a recipient, **DO NOT create new ones** without deleting the old ones or explicit user override.
2.  **Zero-Duplicate Send Policy:** The sending script must enforce a strict **One-Email-Per-Recipient-Per-Day** rule.
    -   *Logic:* querying `messages.list(q="to:EMAIL after:TODAY")` before sending.
    -   *Logic:* querying `drafts.list(q="to:EMAIL")` to ensure we are not sending a backlog of multiple accumulated drafts.
3.  **Purge Stale Drafts:** If a "revised" batch is requested, the previous batch MUST be programmatically deleted first.
