    ...
    for i, row in df.iterrows():
        ...
        for m in case_pattern.finditer(text_str):
            ...
            if case in valid_cases:
                ...
                if match_req:
                    existing = normalize_zone(match_req.group(1))
                    requested = normalize_zone(match_req.group(2))
                else:
                    match_from = pattern_from_to.search(window)
                    if match_from:
                        existing = normalize_zone(match_from.group(1))
                        requested = normalize_zone(match_from.group(2))
                    else:
                        match_prop = pattern_proposed.search(window)
                        if match_prop:
                            requested = normalize_zone(match_prop.group(1))
                
                match_staff = pattern_staff.search(window)
                if match_staff:
                    staff = normalize_zone(match_staff.group(1))
                    if staff == 'NO':
                        staff = None
                ...
                if phase == "Council":
                    ...
                    if matches:
                        final_zoning = normalize_zone(matches[-1])
