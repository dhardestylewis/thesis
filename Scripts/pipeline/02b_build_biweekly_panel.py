...
EARS_DIR      = os.path.join(BASE, "Raw","EARS")
...
z = z.rename(columns={"application_start_date": "App_Date", "tcad_id": "parcel_id_10"})
...
z["T0"]     = z["App_Date"]
z["T_vote"] = pd.NaT
for col in ["day_approved", "approval_date", "final_date", "status_date"]:
    if col in z.columns:
        z["T_vote"] = z["T_vote"].fillna(pd.to_datetime(z[col], errors="coerce"))
z["T_vote"] = pd.to_datetime(z["T_vote"])
...
if "label_real_days_in_pipeline" in z.columns:
    z["label_real_days_in_pipeline"] = z["label_real_days_in_pipeline"].fillna(
        np.minimum((CUTOFF - z["T0"]).dt.days, CENSOR_WINDOW))
else:
    z["label_real_days_in_pipeline"] = np.where(
        z["T_vote"].notna(), (z["T_vote"]-z["T0"]).dt.days,
        np.minimum((CUTOFF - z["T0"]).dt.days, CENSOR_WINDOW))

# Drop data entry typoes (e.g. negative pipeline days, or 1970 epoch errors)
z = z[(z["label_real_days_in_pipeline"] >= 0) & (z["label_real_days_in_pipeline"] <= MAX_DAYS)].copy()
...
for yr in range(2018, 2026):
    fp = os.path.join(EARS_DIR, f"ears_{yr}.csv")
...
for yr, grp in unique_cy.groupby("vintage_year_ears"):
    if yr >= 2018 and yr in ears:
        ref = ears[yr]; ref_type = "ears"
    elif yr >= 2018 and ears:
    ...
    if ref_type == "ears":
        ...
        ref["account_number"] = ref["account_number"].astype(str).str.zfill(10)
...
if os.path.exists(FRED_PATH):
    ...
    panel["period_start"] = pd.to_datetime(panel["period_start"]).astype("datetime64[ns]")
    ...
    fred_lag = fred.copy()
    fred_lag["observation_date"] = (fred_lag["observation_date"] + pd.DateOffset(years=1)).astype("datetime64[ns]")
