...
import csv
PERMIT_PATH = os.path.join(BASE, "CoA_Open_Data", "Issued_Building_Permits.csv")

def load_permits_for_phases():
    print("Loading Building Permits for Lifecycles...")
    permits = {} # tcad -> {issue_date: min, final_date: max, has_expired: bool}
    with open(PERMIT_PATH, 'r', encoding='utf-8', errors='replace') as f:
        ...
        for row in reader:
            if len(row) <= max(tcad_idx, issue_idx, final_idx, type_idx, status_idx): continue
            ...
            issue_d = pd.to_datetime(row[issue_idx], errors='coerce') if row[issue_idx] else pd.NaT
            final_d = pd.to_datetime(row[final_idx], errors='coerce') if row[final_idx] else pd.NaT
            status = str(row[status_idx]).upper().strip()
            
            if tcad not in permits:
                permits[tcad] = {'issue_date': pd.NaT, 'final_date': pd.NaT, 'statuses': set()}
            
            permits[tcad]['statuses'].add(status)
            if pd.notna(issue_d):
                if pd.isna(permits[tcad]['issue_date']) or issue_d < permits[tcad]['issue_date']:
                    permits[tcad]['issue_date'] = issue_d
            if pd.notna(final_d):
                if pd.isna(permits[tcad]['final_date']) or final_d > permits[tcad]['final_date']:
                    permits[tcad]['final_date'] = final_d
    return permits

# ── 1. Zoning cases ────────────────────────────────────────────────────────
print("Step 1: Zoning cases & Permits...")
...
if hasattr(z["App_Date"], "dt") and hasattr(z["App_Date"].dt, "tz_localize"):
    z["App_Date"] = z["App_Date"].dt.tz_localize(None)

if "Final_Council_Date" in z.columns:
    z["Final_Council_Date"] = pd.to_datetime(z["Final_Council_Date"], errors="coerce")
    if hasattr(z["Final_Council_Date"], "dt") and hasattr(z["Final_Council_Date"].dt, "tz_localize"):
        z["Final_Council_Date"] = z["Final_Council_Date"].dt.tz_localize(None)
else:
    z["Final_Council_Date"] = pd.NaT
z = z[z["case_number"].notna() & z["App_Date"].notna()].copy()
z = z.drop_duplicates(subset=["case_number"])
z["T0"] = z["App_Date"]
z["T_vote"] = z["Final_Council_Date"]
z["parcel_id_10"] = z["parcel_id_10"].map(safe_pid)
z["filing_year"] = z["T0"].dt.year

permits = load_permits_for_phases()
z["permit_issue_date"] = z["parcel_id_10"].map(lambda x: permits.get(x, {}).get("issue_date", pd.NaT) if pd.notna(x) else pd.NaT)
z["permit_final_date"] = z["parcel_id_10"].map(lambda x: permits.get(x, {}).get("final_date", pd.NaT) if pd.notna(x) else pd.NaT)
z["permit_has_expired"] = z["parcel_id_10"].map(lambda x: 1 if "EXPIRED" in permits.get(x, {}).get("statuses", set()) else 0)

# EXTEND LIFECYCLE TO 2026 FOR ALL CASES
z["T_end"] = pd.Timestamp(CUTOFF).tz_localize(None)

# ── 2. Skeleton ────────────────────────────────────────────────────────────
print("Step 2: Building Biweekly Lifecycle Skeleton...")
rows = []
for _, r in z.iterrows():
    ...
    app_date = r["T0"]
    issue_date = r["permit_issue_date"]
    if pd.notna(issue_date): issue_date = pd.Timestamp(issue_date).tz_localize(None)
    final_date = r["permit_final_date"]
    if pd.notna(final_date): final_date = pd.Timestamp(final_date).tz_localize(None)
    is_abandoned_flag = 1 if (r["permit_has_expired"] == 1 and pd.isna(final_date)) or (pd.isna(issue_date) and ((z["T_end"].iloc[0] if isinstance(z["T_end"], pd.Series) else z["T_end"]) - app_date).days > 1825) else 0

    for i, ps in enumerate(periods):
        pe = ps + pd.Timedelta(days=13)
        
        # Lifecycle logic
        p_ent = p_exec = p_stab = p_post = p_abnd = 0
        if is_abandoned_flag:
            if (ps - app_date).days < 1825: p_ent = 1
            else: p_abnd = 1
        else:
            if pd.notna(issue_date) and pd.notna(final_date):
                if ps < issue_date: p_ent = 1
                elif issue_date <= ps <= final_date: p_exec = 1
                elif final_date < ps <= final_date + pd.DateOffset(years=2): p_stab = 1
                elif ps > final_date + pd.DateOffset(years=2): p_post = 1
            elif pd.notna(issue_date) and pd.isna(final_date):
                if ps < issue_date: p_ent = 1
                else: p_exec = 1
            else:
                p_ent = 1 # Edge case
        ...
        rows.append({
            ...
            "vote_event": int(pd.notna(r["T_vote"]) and ps <= r["T_vote"] <= pe),
            "resolved": int(pd.notna(r["T_vote"]) and pe >= r["T_vote"]),
            "phase_entitlement": p_ent,
            "phase_execution": p_exec,
            "phase_stabilization": p_stab,
            "phase_post_stabilization": p_post,
            "phase_abandoned": p_abnd,
        })
panel = pd.DataFrame(rows)
print(f"  {len(panel):,} rows | {panel['case_number'].nunique():,} cases | {len(panel)/panel['case_number'].nunique():.1f} periods/case")
...
pet_case_agg["label_valid_protest"] = (pet_case_agg["label_petition_total_pct"] >= 20).astype(int)
...
if "council_district" in panel.columns:
    ...
    case_pet = case_pet.merge(panel[["case_number","label_valid_protest"]].drop_duplicates(),
                               on="case_number", how="left")
...
try:
    ...
    coords_full = (z[["case_number","latitude","longitude","filing_year"]]
                    .merge(pet_case_agg[["case_number","label_valid_protest"]], on="case_number", how="left"))
...
# ── 11. Target Construction for Causal Modeling ─────────────────────────────
print("Step 11: Constructing targets (net_height_change, lagged cumulatives)...")
panel = panel.sort_values(['case_number', 'period_seq'])

# Lagged cumulative features
if 'council_hearings_this_period' in panel.columns:
    panel['cumulative_council_hearings_lag1'] = panel.groupby('case_number')['council_hearings_this_period'].apply(lambda x: x.shift(1).fillna(0).cumsum()).reset_index(level=0, drop=True)
if 'commission_hearings_this_period' in panel.columns:
    panel['cumulative_commission_hearings_lag1'] = panel.groupby('case_number')['commission_hearings_this_period'].apply(lambda x: x.shift(1).fillna(0).cumsum()).reset_index(level=0, drop=True)
...
if "pdf_requested_height_ft" in panel.columns:
    ...
    final_ht = panel["pdf_reduced_to_ft"].fillna(current_constraint).fillna(0) if "pdf_reduced_to_ft" in panel.columns else current_constraint.fillna(0)
...
OUT_COLS = [
    ...
    # Lifecycle Phases
    "phase_entitlement","phase_execution","phase_stabilization","phase_post_stabilization","phase_abandoned",
...
out_path = os.path.join(OUT_DIR, "biweekly_cradle_to_grave_panel.csv")
