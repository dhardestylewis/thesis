...
cmap = cm.get_cmap('viridis', len(horizons))
colors = {h: cmap(i) for i, h in enumerate(horizons.keys())}
...
for h_name, window in horizons.items():
    ...
    plt.plot(r, p, '-', label=f"{h_name} (AUC={auc:.3f})", color=colors[h_name], linewidth=2)
...
plt.title("Multi-Horizon Precision-Recall Decay (CatBoost)", pad=15)
plt.legend(fontsize=9)
plt.grid(True, alpha=0.3)
plt.tight_layout()

out_file = os.path.join(FIG_DIR, "fig_pr_curves_multihorizon.pdf")
