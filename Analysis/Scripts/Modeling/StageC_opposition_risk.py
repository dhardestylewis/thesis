...
def compute_fnr_gap(y_true, y_prob, districts):
    ...
    threshold = np.percentile(y_prob, 50) # Assuming active threshold at median for deployment binary
...
def extract_advanced_metrics(y_true, y_pred, districts=None, name=""):
    if len(np.unique(y_true)) < 2:
        return {'Model': name, 'PR-AUC': np.nan, 'Top-Decile Lift': np.nan, 'ECE': np.nan, 'Brier': np.nan, 'Calib-Slope': np.nan, 'FNR-Gap%': np.nan}
...
def run_bounded_optimization(X, y, sample_weights):
    ...
    if best_model is None:
        print("    [!] No configuration perfectly satisfied [0.9, 1.1] bounds. Relaxing limits to pick best PR-AUC.")
        best_cb = CatBoostClassifier(iterations=150, depth=6, verbose=0, random_seed=42)
        best_model = CalibratedClassifierCV(estimator=best_cb, method='isotonic', cv=5)
    else:
...
def process_horizon(path, horizon_name):
    ...
    if os.path.exists(STAGE_A_PROBS):
        print("[+] IPW: Ingesting Stage A Hazard Probabilities...")
        df_hazard = pd.read_csv(STAGE_A_PROBS, usecols=['standardized_tcad_id', 'year', 'Prob_H=4'])
        if 'standardized_tcad_id' in df.columns:
            ...
            df['ipw'] = 1.0 / np.clip(df['Prob_H=4'].fillna(0.01), 0.0001, 1.0)
    ...
    drop_cols = ['is_protested', 'case_number', 'organized_opposition', 'has_audio_record', 'TCAD ID', 'date', 'application_start_date', 'final_date', 'standardized_tcad_id', 'Prob_H=4', 'ipw', dist_col, 'council_district']
