...
def run():
    ...
    feature_candidates = ['gross_site_area_acres', 'year', 'delta_max_far',
    ...
    for seed in SEEDS:
        ...
        # CatBoost
        cb = CatBoostClassifier(iterations=100, depth=4, learning_rate=0.08,
        ...
        # LightGBM
        lgbm = lgb_lib.LGBMClassifier(n_estimators=100, max_depth=6,
        ...
        p_lr = lr.predict(Xte_s)
        pp_lr = lr.predict_proba(Xte_s)

        for mname, preds, probs in [('CatBoost', p_cb, pp_cb),
                                     ('Random Forest', p_rf, pp_rf),
                                     ('LightGBM', p_lgb, pp_lgb),
                                     ('Logistic Regression', p_lr, pp_lr)]:
            ...
            results[mname]['macro_f1'].append(mf1)
            results[mname]['weighted_f1'].append(wf1)
            results[mname]['accuracy'].append(acc)
            results[mname]['log_loss'].append(ll)

            # per-class
            for tier in TIER_LABELS:
                f1_t = f1_score(y_test == tier, preds == tier, zero_division=0)
                per_class[mname][tier].append(f1_t)

        print(f"  Seed {seed}: " + " | ".join(
            f"{m}={results[m]['macro_f1'][-1]:.3f}" for m in models_cfg))
        gc.collect()

    # Print summary
    print(f"\n  {'Model':<22} {'Macro-F1':>16} {'Weighted-F1':>16} {'Accuracy':>16} {'Log-Loss':>16}")
    print(f"  {'-'*86}")
    for m in models_cfg:
        mf = np.array(results[m]['macro_f1'])
        wf = np.array(results[m]['weighted_f1'])
        ac = np.array(results[m]['accuracy'])
        ll = np.array(results[m]['log_loss'])
        print(f"  {m:<22} {np.mean(mf):.3f}+/-{np.std(mf):.3f}  "
              f"{np.mean(wf):.3f}+/-{np.std(wf):.3f}  "
              f"{np.mean(ac):.3f}+/-{np.std(ac):.3f}  "
              f"{np.nanmean(ll):.3f}+/-{np.nanstd(ll):.3f}")

    # ===== PLOT 1: Aggregate metrics box plots =====
    fig, axes = plt.subplots(1, 4, figsize=(20, 5.5))
    metric_labels = ['Macro-F1', 'Weighted-F1', 'Accuracy', 'Log-Loss']
    metric_keys = ['macro_f1', 'weighted_f1', 'accuracy', 'log_loss']

    for ax, ml, mk in zip(axes, metric_labels, metric_keys):
        data = [results[m][mk] for m in models_cfg]
        bp = ax.boxplot(data, labels=[m.replace(' ', '\n') for m in models_cfg],
                       patch_artist=True, widths=0.6)
        for patch, m in zip(bp['boxes'], models_cfg):
            patch.set_facecolor(colors[m]); patch.set_alpha(0.7)
        for patch in bp['medians']:
            patch.set_color('white'); patch.set_linewidth(2)
        for i, m in enumerate(models_cfg):
            jitter = np.random.normal(0, 0.04, size=len(results[m][mk]))
            ax.scatter(np.ones(len(results[m][mk])) * (i+1) + jitter,
                      results[m][mk], color='black', alpha=0.3, s=12, zorder=5)
        ax.set_title(ml, fontsize=13, pad=8)
        ax.grid(axis='y', linestyle='--', alpha=0.5)

    plt.suptitle("Stage B: 6-Tier Zoning Typology Seed Stability (10 Seeds x 4 Models)",
                 fontsize=14, y=1.01)
    plt.tight_layout()
    p1 = os.path.join(out_dir, "fig_stage_b_seed.pdf")
    plt.savefig(p1, bbox_inches='tight', dpi=150)
    print(f"\n[+] Saved: {p1}")
    plt.close()

    # ===== PLOT 2: Per-class F1 heatmap across seeds =====
    fig, axes = plt.subplots(1, len(models_cfg), figsize=(5*len(models_cfg), 5))

    for ax, mname in zip(axes, models_cfg):
        matrix = np.array([per_class[mname][t] for t in TIER_LABELS])  # 6 x 10
    ...
    plt.suptitle("Stage B Per-Class F1 Stability Across Seeds", fontsize=14, y=1.02)
    plt.tight_layout()
    p2 = os.path.join(out_dir, "fig_stage_b_perclass.pdf")
