...
% It describes the panel construction, sources, and temporal leakage audit.
...
Following Peters et al.\ (2016), calendar years are treated as distinct environments
...
\begin{enumerate}
 ...
 The sample contains 27,363 property-petition records. Our binary outcome variable
 ...
 A 72.7\% match rate on eligible property-years is achieved via an ID crosswalk
 ...
 Pulled from the Census Bureau API for all 15 available using tidycensus. Decennial census boundaries mapped via weighted areal interpolation
 if boundaries drifted between 2010 and 2020.
\end{enumerate}
...
(e.g., 237076) with zero direct overlap. This is resolved through the
...
A forward-fill strategy is applied: for each property-year, the most recent
snapshot value observed at or before the panel year. For panel years preceding the
earliest snapshot, it is backward-filled from the first available observation.
...
The corresponding metrics are detailed in \autoref{tab:forwardfill}.
...
\begin{description}
 ...
 only lagged ($t-1$) values are used or time-since-sale features are derived.
 Similarly, \texttt{zoning\_code} may change as a \textit{result} of the zoning case
 under protest, so prior-year values are used exclusively.
...
Following Peters et al.\ (2016) and Bühlmann (2020), calendar years are treated as
