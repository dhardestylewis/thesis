...
print("[*] Processing Patched Universal GeoAgg Matrix...")
...
for scale_name, geo_col in geo_scales.items():
    print(f"---> Aggregating Strict Chronology: {scale_name}")
    ...
    # Grab y+1, if it doesn't exist, evaluates to NaN cleanly
    agg_feats['Target_1yr'] = agg_feats.apply(lambda r: target_dict.get((r[geo_col], r['year'] + 1), np.nan), axis=1)
    ...
    for anchor in anchors:
        ...
        results.append({
            ...
            'Anchor': f"Pre-{anchor}",
            'Mean_MdAPE': round(mean_mdape, 3) if not np.isnan(mean_mdape) else None
        })

# Absolute Individual Baseline loop
m = LGBMRegressor(n_estimators=100, max_depth=6, random_state=42, objective='poisson', verbose=-1)
...
for anchor in anchors:
    ...
    m.fit(X_ind[train_mask], y_ind[train_mask])
    anchor_mdape = []
...
print("\n=== Fixed Patched GeoAgg Bounds (Explicit Chronology) ===")
print("Tracking Mean Percentage Error computed seamlessly across all forward years.")
pivot = res_df.pivot_table(index='Geographic_Layer', columns='Anchor', values='Mean_MdAPE')
