...
total_app = len(df)
dead_pre = len(df[df['Derived_Status'] == 'Dead (Pre-Council)'])
dead_council = len(df[df['Derived_Status'] == 'Dead (At Council)'])
ongoing_pre = len(df[df['Derived_Status'] == 'Ongoing (Pre-Council)'])
...
total_dead = dead_pre + dead_council
total_ong = ongoing_pre + ongoing_council
...
In this version, the governing bodies are static, singular nodes. The physical friction is represented purely by the edges looping back upon the node, and the **Attrition** is correctly split between the Commission and Council levels.

```mermaid
graph LR
    APP["Application Filed<br>N={{total_app:,}}"]
    COMM["Planning / ZAP Commission<br>N={{total_app:,}}"]
    ...
    %% Initial Flows
    APP -->|"{{total_app:,}}"| COMM
    COMM -->|"{{dead_pre:,}} (Died Pre-Council)"| DEAD
    COMM -->|"{{ongoing_pre:,}} (Stalled Pre-Council)"| ONG
...
> [!NOTE]
> This artifact is programmatically generated. Every single N= label is a mathematical output directly aggregated from the case datasets.
"""
