...
dead_pre = len(df_modern[df_modern['Derived_Status'] == 'Dead (Pre-Council)'])
dead_council = len(df_modern[df_modern['Derived_Status'] == 'Dead (At Council)'])
ongoing_pre = len(df_modern[df_modern['Derived_Status'] == 'Ongoing (Pre-Council)'])
...
# Commission routes volume to Dead(Pre), Ongoing(Pre), and Council
...
source = [0, 1, 1, 1, 4, 4, 4, 4]
target = [1, 2, 3, 4, 5, 2, 3, 6]
value  = [
    total_applied,  # App -> Comm
    dead_pre,       # Comm -> Dead
    ongoing_pre,    # Comm -> Ongoing
