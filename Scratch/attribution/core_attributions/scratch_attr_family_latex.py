...
tex_lines = [
    ...
    r'\caption[Archetypal Family Attribution]{\textbf{Archetypal Family Attribution.} Average percentage of absolute model reliance allocated to each semantic feature cluster, highlighting the structural divergence between tree ensembles (socio-demographic focus) and deep architectures (physical-scale focus).}',
    r'\label{tab:archetypal_attribution}',
    r'\begin{tabular}{lcc}', r'\toprule',
    r'\textbf{Semantic Target Cluster} & \textbf{Tree Ensembles} & \textbf{Deep Architectures} \\',
    r'\midrule'
]

# Sort by Trees importance descending
tree_means = [np.mean(trees[:, i]) for i in range(len(cols))]
deep_means = [np.mean(np.concatenate([deep_erm[:, i], deep_vrex[:, i]])) for i in range(len(cols))]

sorted_indices = np.argsort(tree_means)[::-1]
...
for i in sorted_indices:
    ...
    # Bold the dominant family for that metric
    if t > d:
        t_str = f"\\textbf{{{t_str}}}"
    else:
        d_str = f"\\textbf{{{d_str}}}"
        
    tex_lines.append(f"{c} & {t_str} & {d_str} \\\\")

tex_lines.extend([r'\bottomrule', r'\end{tabular}', r'\end{table}'])

OUT_DIR = os.path.join(ROOT, 'Thesis_Draft', 'Draft_v1', 'Tables')
with open(os.path.join(OUT_DIR, 'archetypal_attribution.tex'), 'w') as f:
    f.write('\n'.join(tex_lines))
