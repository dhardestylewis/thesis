...
# NOTE: Load the freshly minted Deep Macro-Momentum Lagged Dataset exclusively
df = pd.read_csv(os.path.join(DATA, 'H0_Filing_Master_Enriched_Lagged.csv'), low_memory=False)
...
print("[*] Launching Master Recursive Meta-Attribution securely mapped against deep 6-year macro momentum constraints natively...")
...
print("[*] Normalizing 2D matrix arrays natively mapping explicit deep limits safely...")
...
out_csv = os.path.join(DRAFT_DIR, "Recursive_LTR_Clustermap.csv")
df_map.to_csv(out_csv, index=True)
print(f"Dumped fully normalized Deep Lag Recursive Clustermap arrays softly cleanly to: {out_csv}")
...
cg = sns.clustermap(
    ...
    linewidths=.5, 
    annot=False, 
    cbar_kws={'label': 'NDCG-Weighted Relational Importance (%) (Recursive)'}
)
cg.fig.suptitle("Recursive Meta-Meta Attribution (Deep 6-Year Lags)\nEmpirically tracing unwrapped failures explicitly checking 1-to-6 year macro historical topological drift limits", 
                fontsize=16, weight='bold', y=1.02)

out_png = os.path.join(DRAFT_DIR, "plot_recursive_clustermap.png")
cg.savefig(out_png, dpi=300, bbox_inches='tight')
plt.close()
print(f"Dumped Master Recursive Heatmap arrays tracking dynamic deep lag parameters out to: {out_png}")
