...
# Filter for the important document types
mask = df_all['Doc_Text'].str.contains('Staff Report|Opposition|Public Comment|Support', case=False, na=False)
df_target = df_all[mask].copy()

# Deduplicate by URL just in case
df_target = df_target.drop_duplicates(subset=['Doc_URL'])
...
for idx, row in df_target.iterrows():
    ...
    # Truncate clean_name if it's too long to avoid OS path length limits
...
def download_file(task):
    ...
    # Simple retry logic for fragility of EDIMS
    for _ in range(3):
        try:
...
print("Starting concurrent download with 10 workers...", flush=True)
