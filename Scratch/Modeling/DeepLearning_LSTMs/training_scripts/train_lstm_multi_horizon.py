...
for name, window in horizons.items():
    ...
    model = LSTMHazardModel(input_dim=len(FEATS), hidden_dim=256, num_layers=2, dropout=0.1).to(device)
    try:
...
print("\nSaving LSTM Gravity Curve...", flush=True)
res_df = pd.DataFrame(results)
res_df.to_csv("lstm_horizon_results.csv", index=False)

sns.set_theme(style="whitegrid", palette="mako")
fig, ax1 = plt.subplots(figsize=(10, 6))

sns.lineplot(data=res_df, x="Horizon", y="PR_AUC", marker="s", linewidth=3, ax=ax1, color="#005b96", label="LSTM PR AUC")
...
plt.title("Sequence Model (LSTM) Performance Across Horizons", fontsize=16, fontweight='bold', pad=20)
plt.tight_layout()
plt.savefig(r"C:\Users\dhl\.gemini\antigravity\brain\1c4648c0-f36a-4614-a8f1-c9e2e5621756\lstm_gravity_curve.png", dpi=300, bbox_inches='tight')
