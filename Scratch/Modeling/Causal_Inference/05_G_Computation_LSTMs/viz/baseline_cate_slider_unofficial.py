...
Y = cs['concession_binary'].values

thresholds = [0.001] + list(np.arange(0.05, 1.05, 0.05))
surfaces = []

print('Computing CATE surfaces across thresholds...')
for t in thresholds:
...
if valid_cates:
    ...
    z_min, z_max = all_cates.min(), all_cates.max()
else:
    z_min, z_max = -0.1, 0.1
...
for i, surf in enumerate(surfaces):
    ...
    step = dict(
        ...
        args=[{'visible': [False] * len(surfaces)},
              {'title': f'Spatial CATE Surface for Petition Dose >= {thresh:.3f}'}],
...
if surfaces:
    ...
    fig.update_layout(
        sliders=sliders,
        title=f'Spatial CATE Surface for Petition Dose >= {first_t:.3f}',
        scene=dict(
            xaxis_title='Longitude',
            yaxis_title='Latitude',
            zaxis_title='Treatment Effect (CATE)',
    ...
    out_path = r'C:\Users\dhl\.gemini\antigravity\brain\1632e32a-ef31-4422-854b-ea7296224fe1\cate_spatial_dose_slider_unofficial.html'
    fig.write_html(out_path)
    print(f'\nSaved Slider HTML to {out_path}')
