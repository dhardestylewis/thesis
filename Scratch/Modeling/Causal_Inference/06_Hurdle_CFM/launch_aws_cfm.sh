...
LOCAL_DIR="$(pwd)"
DATA_FILE="../../../../Data/Panel/biweekly_panel_patched.csv"

echo "1. Syncing python scripts to S3..."
aws s3 cp "${LOCAL_DIR}/causal_cfm_cvae.py" "${S3_BUCKET}/src/causal_cfm_cvae.py"
aws s3 cp "../05_G_Computation_LSTMs/aws_deploy/Dockerfile" "${S3_BUCKET}/src/Dockerfile"
aws s3 cp "../05_G_Computation_LSTMs/aws_deploy/requirements.txt" "${S3_BUCKET}/src/requirements.txt"

echo "2. Syncing dataset to S3..."
aws s3 cp "$DATA_FILE" "${S3_BUCKET}/data/biweekly_panel_patched.csv"
...
aws s3 cp ${S3_BUCKET}/data/biweekly_panel_patched.csv biweekly_panel_patched.csv

# Build Container
sudo docker build -t causal_pipeline .

# Start a background process to continuously sync logs to S3 every 60 seconds
aws s3 cp /dev/null ${S3_BUCKET}/logs/training_cfm.log
(
  while true; do
    if [ -f "$OUT_DIR/training_cfm.log" ]; then
      aws s3 cp $OUT_DIR/training_cfm.log ${S3_BUCKET}/logs/training_cfm.log > /dev/null 2>&1
    fi
    sleep 60
  done
) &
...
for FOLD in {0..8}; do
    echo "Starting Fold $FOLD..." | tee -a $OUT_DIR/training_cfm.log
    sudo docker run --gpus all --ipc=host \
        -v $(pwd):/data \
        -e PANEL_PATH=/data/biweekly_panel_patched.csv \
        -e OUT_DIR=/data/output \
        causal_pipeline python causal_cfm_cvae.py --fold $FOLD 2>&1 | tee -a $OUT_DIR/training_cfm.log
done
...
if [ -f "$OUT_DIR/vae_dose_response_surface_expanded.csv" ]; then
    aws s3 cp $OUT_DIR/vae_dose_response_surface_expanded.csv ${S3_BUCKET}/output/vae_dose_response_surface_expanded_patched.csv
fi
aws s3 sync $OUT_DIR/ ${S3_BUCKET}/output/ --exclude "*" --include "*.pt"

# Always upload the logs to S3
aws s3 cp $OUT_DIR/training_cfm.log ${S3_BUCKET}/logs/training_cfm_final.log
