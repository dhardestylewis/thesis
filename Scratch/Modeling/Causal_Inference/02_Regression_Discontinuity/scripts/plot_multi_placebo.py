"""
Plot Multi-Target Threshold Test
Calculates the Regression Discontinuity ATE jump at every 1% threshold for 7 targets
and plots them in a 2x4 grid to visually prove the 20% peak effect across metrics.
"""
import pandas as pd
import numpy as np
...
def run_rd_cutoff(cutoff, t_col):
    ...
    # Need at least 2 points on each side to calculate a mean without blowing up
...
targets = {
    "t_withdrawal": ("Developer Withdrawal", "prob"),
    "t_denial": ("Active Denial", "prob"),
    ...
    "t_max_nay_votes": ("Max Nay Votes", "count_v")
}
...
for i, (t_col, (t_name, t_type)) in enumerate(targets.items()):
    ates = [run_rd_cutoff(c, t_col) for c in cutoffs]
    ax = axes[i]
    
    # Mask out extreme noise above 70% if it's completely unstable due to sample size (N=1)
    # We will plot it but use low alpha for the >50% region where data is extremely sparse
    ax.plot(cutoffs[:50], ates[:50], color='firebrick', marker='o', linewidth=2, markersize=4)
    ax.plot(cutoffs[50:], ates[50:], color='firebrick', marker='o', linewidth=1, markersize=2, alpha=0.3, label='Sparse Data (>50%)')

    ax.axvline(x=20, color='black', linestyle='--', linewidth=1.5, label='20% Legal Threshold')
    
    # Highlight peak magnitude
    valid_ates = [a for a in ates[:50] if not np.isnan(a)] # Only look for peaks in the reliable data region
    if valid_ates:
        ...
        ax.text(cutoffs[peak_idx], y_text, f"{peak_val:+.3f}", ha='center', va='center', fontweight='bold', fontsize=10, bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=1))
    ...
    if i % 4 == 0: ax.set_ylabel("Discontinuous Jump (ATE)")
    ax.grid(alpha=0.2)
...
plt.suptitle("Threshold Robustness Test (1% to 100%)", fontsize=16, fontweight='bold')
sns.despine()
plt.tight_layout(rect=[0, 0, 1, 0.96])

out_path = rf"{OUT_DIR}\multi_target_threshold_robustness.png"
