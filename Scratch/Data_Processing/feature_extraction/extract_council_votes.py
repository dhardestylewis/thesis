...
agendas_csv = r"c:\Users\dhl\data\Thesis\thesis\Data\council_agendas_cases.csv"
...
# Group by Doc_ID to avoid opening the same PDF multiple times
grouped = df_merged.groupby(['Doc_ID', 'Year'])

print(f"Starting extraction for {len(df_merged)} case appearances across {len(grouped)} Minutes PDFs...", flush=True)
...
df_out = pd.DataFrame(extracted_votes)
df_out = df_out.drop_duplicates()
df_out.to_csv(output_csv, index=False)

elapsed = time.time() - start_time
print(f"\nExtraction complete in {elapsed:.2f} seconds!", flush=True)
print(f"Saved {len(df_out)} vote transcripts to {output_csv}.", flush=True)
