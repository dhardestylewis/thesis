    ...
    for m in case_pattern.finditer(text_str):
        ...
        if case in valid_cases:
            ...
            if requested or staff or existing:
                event = {"phase": "Commission", "date": date_val}
...
for i, row in df_council.iterrows():
    ...
    for m in case_pattern.finditer(text_str):
        ...
        if case in valid_cases:
            ...
            existing = None
            requested = None
            match_req = pattern_from_to.search(window)
            if match_req:
                existing = match_req.group(1).upper()
                requested = match_req.group(2).upper()
            else:
                match_req2 = pattern_req_to.search(window)
                if match_req2:
                    existing = match_req2.group(1).upper()
                    requested = match_req2.group(2).upper()
                else:
                    match_prop = pattern_proposed.search(window)
                    if match_prop:
                        requested = match_prop.group(1).upper()
            ...
            if requested or existing:
                event = {"phase": "Council", "date": date_val}
                if existing: event["existing_zoning"] = existing
                if requested: event["requested_zoning"] = requested
                trajectory_dict[case].append(event)
    ...
    if len(events) > 0:
        ...
        for e in events:
            if not clean_events:
                ...
                prev = clean_events[-1]
                if e.get('phase') == prev.get('phase') and e.get('requested_zoning') == prev.get('requested_zoning') and e.get('approved_zoning') == prev.get('approved_zoning') and e.get('staff_recommendation') == prev.get('staff_recommendation') and e.get('existing_zoning') == prev.get('existing_zoning'):
                    continue
                clean_events.append(e)
                
        df_model.at[idx, 'Zoning_Trajectory'] = json.dumps(clean_events)
        
        existings = [e.get('existing_zoning') for e in clean_events if e.get('existing_zoning')]
        reqs = [e.get('requested_zoning') for e in clean_events if e.get('requested_zoning')]
        finals = [e.get('approved_zoning') for e in clean_events if e.get('approved_zoning')]
