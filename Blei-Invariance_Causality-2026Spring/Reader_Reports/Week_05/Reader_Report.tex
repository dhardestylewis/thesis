...
Two complementary methods recover $S^*,\beta^*$. \citet{fan2024negdro} cast the problem as a minimax optimization: $\hat\beta^{\gamma}\in\arg\min_{b}\max_{w\in U(\gamma)}\sum_{e} w_e\,\hat{\mathbb{E}}[(Y^e{-}b^\top X^e)^2]$, where $U(\gamma)$ allows \emph{negative} weights to enforce risk parity; under Condition~4, this recovers $\beta^*$ even with limited interventions. \citet{wu2025bayesian} take a Bayesian route: a latent selector $z\in\{0,1\}^p$ with posterior $p(z\mid\mathcal{D})\propto p(z)\prod_{e,i}\hat g(Y^e_i\mid X^z_{ei})/\hat p_e(Y^e_i\mid X^z_{ei})$ concentrates on $z^*$ as data and environments grow, while quantifying remaining ambiguity.

\vspace{-0.5em}
{\footnotesize\setlength{\bibsep}{0pt}
