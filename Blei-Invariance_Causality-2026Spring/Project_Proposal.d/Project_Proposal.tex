...
\pagestyle{fancy}
\fancyhf{}
\lhead{STAT8101: Invariance, Causality, and Applications}
\rhead{Columbia University --- David M. Blei (db2128)}
\cfoot{\thepage}

\title{\vspace{-2em}\textbf{Predicting NIMBYism: An Invariant Perspective}}
\author{Daniel Hardesty Lewis (dl3645)\vspace{-1ex}}
\date{2026 Spring}
...
Using \textbf{Invariant Risk Minimization (IRM)} [Arjovsky et al., 2019, p. 5] and \textbf{Bayesian Invariant Prediction (BIP)} [Wu et al., 2025b, p. 2], both grounded in the seminal \textbf{Invariant Prediction} framework [Peters et al., 2016, p. 947]:
\begin{enumerate}
    ...
    \item \textbf{Methods:} We implement the two core paradigms scheduled for \textbf{Weeks 6--7} of the seminar [Blei, 2026]:
    \begin{itemize}
        \item \textbf{Invariant Risk Minimization (IRM):} A \textit{regularized optimization} paradigm to learn $\Phi$ such that a \textbf{fixed dummy classifier} $w=1.0$ is simultaneously \textbf{optimal} across all $\mathcal{E}_{tr}$, \textbf{enforced} via a \textbf{gradient norm penalty} [Arjovsky et al., 2019, p. 5]:
        \[ \min_{\Phi} \sum_{e \in \mathcal{E}_{tr}} R^e(\Phi) + \lambda \|\nabla_{w|w=1.0} R^e(w \cdot \Phi)\|^2 \]
        \item \textbf{Bayesian Invariant Prediction (BIP):} A \textit{probabilistic inference} paradigm defining a latent \textbf{feature selection vector} $z \in \{0,1\}^p$; the \textbf{posterior $p(z|D)$ targets the true invariant features} [Wu et al., 2025b, p. 2].
    \end{itemize}
\end{enumerate}

\section*{4. Why is this solution good? Where does it fall short?}
\textbf{It falls short because} it assumes an \textbf{invariant causal mechanism} [Peters et al., 2018, p. 107]. The invariance assumption fails if the underlying reasons for NIMBYism undergo \textbf{mechanism change} over time [Peters et al., 2018, p. 107]. For instance, neighborhood opposition may shift from traffic congestion fears to displacement concerns as demographics evolve.
...
\begin{itemize}
    ...
    \item Blei, D. M. (2026). STAT8101: Invariance, Causality, and Applications. \textit{Course Syllabus and Website}, Columbia University.
