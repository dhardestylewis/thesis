...
\begin{enumerate}
    ...
    \item \textbf{Identification Strategy:} Combine \textbf{regularized optimization} (IRM) with \textbf{posterior inference} (BIP):
    \begin{itemize}
        \item \textbf{Regularized Optimization (IRM):} Learn a representation $\Phi$ such that a \textbf{fixed dummy classifier} $w=1.0$ is optimal across all $\mathcal{E}_{tr}$, achieved via a gradient norm penalty [Arjovsky et al., 2019, p. 5]:
        \[ \min_{\Phi} \sum_{e \in \mathcal{E}_{tr}} R^e(\Phi) + \lambda \|\nabla_{w|w=1.0} R^e(w \cdot \Phi)\|^2 \]
        \item \textbf{Posterior Inference (BIP):} Compute the posterior $p(z|D)$ for a latent \textbf{feature selection vector} $z \in \{0,1\}^p$, targeting the features with a stable predictive relationship [Wu et al., 2025b, p. 2].
    \end{itemize}
\end{enumerate}
