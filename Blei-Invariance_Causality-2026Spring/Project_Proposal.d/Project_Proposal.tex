...
\begin{enumerate}
    ...
    \item \textbf{Identification Strategy:} Combine regularized optimization (IRM) with probabilistic inference (BIP):
    \begin{itemize}
        \item \textbf{Optimization (IRM):} Minimize the empirical risk while penalizing the gradient of the loss with respect to a dummy scalar $w=1.0$ [Arjovsky et al., 2019]:
        \[ \min_{\Phi} \sum_{e \in \mathcal{E}_{tr}} R^e(\Phi) + \lambda \|\nabla_{w|w=1.0} R^e(w \cdot \Phi)\|^2 \]
        \item \textbf{Posterior Inference (BIP):} Compute the posterior $p(z|D)$ for a latent feature selector $z$, targeting the set that maximizes predictive invariance across contexts [Wu et al., 2025b].
    \end{itemize}
\end{enumerate}
