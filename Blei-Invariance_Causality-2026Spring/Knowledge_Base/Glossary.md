...
**Intervention ($do(x)$)**
An external action that modifies the mechanism generating a subset of variables in an SCM (Graph Surgery). In Invariant Prediction, we assume interventions occur on variables *other than* the target $Y$.
*   *Grounding:* [Pearl, 2016, Ch 3.1 / Peters et al., 2016, p. 1 / Peters et al., 2018, p. 3175]
...
An alternative **graphical tool** that uses a mediator $M$ to identify effects when the direct path is confounded.
*   *Grounding:* [Pearl, 2016, Def 3.4.1]

**Adjustment Formula (Algebraic Tool)**
The primary **algebraic tool** used to **compute** the causal **effect** $P(Y|do(x))$ from **observational probabilities** by conditioning on a **valid adjustment set** $Z$.
*   *Grounding:* [Pearl, 2016, Eq 3.5]
**Average Causal Effect (ACE)**
The difference in expected outcomes between two interventions, typically $E[Y|do(X=1)] - E[Y|do(X=0)]$.
