...
for i, text in enumerate(df_council['Vote_Transcript']):
    ...
    # Process just like Commission: scan the full text for case numbers
    for m in case_pattern.finditer(text_str):
        case = m.group(1)
        if case in valid_cases:
            if case not in council_dict:
                ...
                if match_from:
                    council_dict[case] = (match_from.group(1).upper(), match_from.group(2).upper())
                    continue
                    
                # Priority 3: "Proposed [Zone]"
                match_prop = pattern_proposed.search(window)
                if match_prop:
...
final_zoning_dict = {}
approval_words = ['APPROVE', 'CONSENT', 'STAFF REC', 'PASSED']
...
for i, text in enumerate(df_council['Vote_Transcript']):
    ...
    for m in case_pattern.finditer(text_str):
        ...
        if case in valid_cases:
            ...
            is_approved = any(word in window for word in approval_words)
            
            if is_approved:
                if case in council_dict and council_dict[case][1] and council_dict[case][1] != 'UNKNOWN':
                    final_zoning_dict[case] = council_dict[case][1]
                elif case in comm_dict and comm_dict[case][1] and comm_dict[case][1] != 'UNKNOWN':
                    final_zoning_dict[case] = comm_dict[case][1]
            
            # Fallback
            if case not in final_zoning_dict:
                matches = pattern_all.findall(window)
                if matches:
                    final_zoning_dict[case] = matches[-1]

for col in ['Initial_Zoning', 'Requested_Zoning', 'Final_Zoning', 'Commission_Requested_Zoning', 'Council_Requested_Zoning']:
...
for idx, row in df_model.iterrows():
    ...
    # Base Initial Zoning is whatever the first appearance says
    ...
    df_model.at[idx, 'Commission_Requested_Zoning'] = c_req if c_req else None
    df_model.at[idx, 'Council_Requested_Zoning'] = cc_req if cc_req else None
